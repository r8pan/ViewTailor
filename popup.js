...
optimization.onclick = element => {
    console.log("received optimization click event");
    let isChecked = document.getElementById("optimization").checked;
    chrome.tabs.query({active: true, currentWindow: true}, tabs => {
        let url = (new URL(tabs[0].url)).hostname;
        chrome.storage.local.get(url, data => {
            if (!data[url]) data[url] = {};
            data[url].optimization = isChecked;
            chrome.storage.local.set({[url]: data[url]}, () => {
                if (isChecked) {
                    fetch('src/WCAG/linkPurposes/linkPurposes.js')
...
// let textScale = document.getElementById("text-scale");
...
fontColor.onclick = () => {
    ...
    if (document.activeElement === fontColor && colorPicker.style.display === "block") {
        ...
        // document.getElementById("colorPicker0").focus();
    }
};
    ...
    chrome.tabs.query({active: true, currentWindow: true}, tabs => {
        ...
        chrome.storage.local.get(url, data => {
            ...
            if (fbgColor === "fontColor") {
                ...
                chrome.storage.local.set({[url]: data[url]}, () => {
                    fetch('src/UAAG/fontColor/fontColor.js')
                        .then(response => response.text())
                        .then(text => {
                            chrome.tabs.executeScript(tabs[0].id,
                                {code: text.substring(text.indexOf("var")), allFrames: true});
                ...
                chrome.storage.local.set({[url]: data[url]}, () => {
                    fetch('src/UAAG/backgroundColor/backgroundColor.js')
                        .then(response => response.text())
                        .then(text => {
                            chrome.tabs.executeScript(tabs[0].id,
                                {code: text.substring(text.indexOf("var")), allFrames: true});
...
//**text scale******************************************************************
// textScale.oninput = function() {
//     console.log("received text scale click event");
//     let newV = this.value;
//     document.getElementById("text-scale-value").textContent = this.value + "%";
//     chrome.tabs.query({active: true, currentWindow: true}, tabs => {
//         chrome.tabs.executeScript(tabs[0].id,
//             {code: "var newValue = " + newV}, () => {
//                 chrome.tabs.executeScript(tabs[0].id,
//                     {file: "src/UAAG/textScale/textScale.js", allFrames: true});
//         });
//     });
// };
...
lineSpacing.oninput = function() {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, tabs => {
        ...
        chrome.storage.local.get(url, data => {
            ...
            chrome.storage.local.set({[url]: data[url]}, () => {
                fetch('src/UAAG/lineSpacing/lineSpacing.js')
                    .then(response => response.text())
                    .then(text => {
                        chrome.tabs.executeScript(tabs[0].id,
                            {code: text.substring(text.indexOf("var")), allFrames: true});
...
focusHighlight.onclick = element => {
    ...
    if (isChecked) {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, tabs => {
            chrome.tabs.executeScript(tabs[0].id,
                {file: "src/UAAG/focusHighlight/enable.js", allFrames: true});
            });
    }
    else {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, tabs => {
            chrome.tabs.executeScript(tabs[0].id,
                {file: "src/UAAG/focusHighlight/disable.js", allFrames: true});
            });
    }
}
...
distinguishVisitedLinks.onclick = element => {
    ...
    if (isChecked) {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, tabs => {
            chrome.tabs.executeScript(tabs[0].id,
                {file: "src/UAAG/distinguishVisitedLinks/enable.js", allFrames: true});
            });
    }
    else {
        console.log("Disabled");
        chrome.tabs.query({active: true, currentWindow: true}, tabs => {
            chrome.tabs.executeScript(tabs[0].id,
                {file: "src/UAAG/distinguishVisitedLinks/disable.js", allFrames: true});
            });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    chrome.tabs.query({active: true, currentWindow: true}, tabs => {
        ...
        chrome.storage.local.get(url, data => {
            ...
            if (data[url]) {
                optimization.checked = data[url].optimization;
                if (data[url].optimization) {
                    console.log("if");
                    chrome.tabs.query({active: true, currentWindow: true}, tabs => {
                        fetch('src/WCAG/linkPurposes/linkPurposes.js')
                            .then(response => response.text())
                ...
                // if (data[url].textScale) {
                //     textScale.value = data[url].textScale;
                //     console.log("on opening popup");
                //     document.getElementById("text-scale-value").textContent = data[url].textScale + "%";
                //     chrome.tabs.query({active: true, currentWindow: true}, tabs => {
                //         chrome.tabs.executeScript(tabs[0].id,
                //             {code: "var newValue = " + data[url].textScale}, () => {
                //                 chrome.tabs.executeScript(tabs[0].id,
                //                     {file: "src/UAAG/textScale/textScale.js", allFrames: true});
                //         });
                //     });
